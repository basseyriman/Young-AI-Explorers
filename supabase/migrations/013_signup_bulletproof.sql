-- Bulletproof signup: missing columns, anon country read, trigger never blocks auth signup
-- Run after 012_signup_trigger_complete.sql
...
BEGIN
  ...
  BEGIN
    ...
    INSERT INTO public.curriculum_settings (user_id, sharing_level, allow_match_quiz)
    ...
    ON CONFLICT (user_id) DO UPDATE SET
      ...
      updated_at = NOW();
  EXCEPTION WHEN OTHERS THEN
    RAISE LOG 'handle_new_user profile setup failed for %: %', NEW.id, SQLERRM;
  END;
